...
我们的仓库只提交自有资产：改写后的提示词（基于上游 MIT 协议文本的演绎，注明归属）、SenseNova 生成的图片、评审 JSON、全部脚本。上游 `cases.json` 与 538 张 GPT-Image-2 参考图**不二次分发**，由 `fetch-reference.sh` 按锁定 commit 从上游拉取至 `third_party/ref/`（gitignore）；README 按 case id + sourceUrl 回链上游署名。涉及真人肖像或品牌标识的上游案例在 SELECT 阶段排除（与基础仓库画廊的无肖像无 logo 实践一致）。
